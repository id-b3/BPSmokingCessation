...
def make_plots(data, bps, out_path):
    ...
    data["age_2yr"] = pd.cut(
        data["age_at_scan"], bins=age_cut_2, labels=age_label_2, right=False
    )
    ...
    for param in bps:
        for gender in ["Male", "Female"]:
            for sm_stat in ["never_smoker", "ex_smoker", "current_smoker"]:
                percentiles = (
                    data[(data["smoking_status"] == sm_stat) & (data["gender"] == gender)]
                    .groupby("age_2yr")[param]
                ...
                fig.savefig(f"{str(out_path / param)}_{gender}_{sm_stat}.png", dpi=300)
                plt.close()
